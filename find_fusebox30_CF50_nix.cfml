...
<!--- 29 Oct 09 amccollough: I modified this file, various locations, commenting out <cfthrow message="#cfcatch.message#" detail="#cfcatch.detail#"> and using <cfrethrow> instead.
Why? The original did not work properly with Railo 3.1. --->
...
------------------------------------------------------------------------------------------->
<!--- -->
<fusedoc fuse="FBX_FUSEBOX301_CF50_NIX.CFM" specification="2.0">
	<responsibilities>
		!! *nix Version !!
...
A boolean variable, which defaults to FALSE. If TRUE, the Fusebox framework will attempt to give you "smarter" errors that may occur from within its own code as it applies to the Framework itself. This is useful for production environments. If FALSE (default), you will receive the native CF error messages. Due to a bug in the cfrethrow tag for the Linux version of ColdFusion, this "_nix" version of this file will not throw native errors, but will instead throw a reformatted version of the native error. During development you may want to turn this to TRUE and FALSE alternately to ensure you've got your framework set up properly. Set this to TRUE in a production enviroment, since at that point errors that occur will not be Fusebox framework errors but rather errors in your fuseactions and fuses.		
	fusebox.Circuits:
...
<cftry>
   <cfinclude template="fbx_Circuits.cfm"> 
	<cfcatch>
		...
		<cfelse><!---<cfthrow message="#cfcatch.message#" detail="#cfcatch.detail#">---><cfrethrow></cfif>
	</cfcatch>
</cftry>
...
<cftry>
	...
	<cfcatch>
		...
		<cfoutput>The variable "attributes.fuseaction" is not available or the Fusebox framework could not find the circuit you requested: "#fusebox.circuit#". If you think this error is incorrect, turn off the Fusebox suppress error messages flag by setting fusebox.SuppressErrors to FALSE, and you will receive ColdFusion's "normal" error output.</cfoutput><!---<cfthrow message="#cfcatch.message#" detail="#cfcatch.detail#">---><cfrethrow><cfabort>
	</cfcatch>
</cftry>
